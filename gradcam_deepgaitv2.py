#!/usr/bin/env python3
"""
Grad-CAM visualization for DeepGaitV2 model
...
class GradCAM:
    """Grad-CAM implementation for DeepGaitV2"""
    
    def __init__(self, model, target_layer):
        """
        Args:
            model: DeepGaitV2 model
            target_layer: Layer to hook for Grad-CAM (e.g., 'layer0', 'layer1', 'layer2', 'layer3', 'layer4')
        """
        self.model = model
    ...
    def hook_layers(self):
        ...
        elif self.target_layer == 'layer2':
            target_module = self.model.layer2
        elif self.target_layer == 'layer3':
            target_module = self.model.layer3
        elif self.target_layer == 'layer4':
            target_module = self.model.layer4
        else:
            raise ValueError(f"Unknown target layer: {self.target_layer}. Options: layer0, layer1, layer2, layer3, layer4")
    ...
    def generate_cam(self, input_tensor, target_class=None):
        ...
        # DeepGaitV2 expects input to be [N, S, H, W] (4D)
        # Ensure input is 4D
        ...
        # Forward pass
        # Prepare input in the format DeepGaitV2 expects
        batch_size = input_tensor.size(0)
        seq_len = input_tensor.size(1)  # S dimension
        
        # Create dummy inputs for DeepGaitV2 forward
        # DeepGaitV2 expects: (ipts, labs, class_id, _, seqL)
        ...
        logits = output['inference_feat']['embeddings']  # [N, C, P] where P is parts_num (16)
...
def load_deepgaitv2_model(config_path, checkpoint_path, device='cuda'):
    """Load DeepGaitV2 model from checkpoint"""
    # Initialize minimal distributed environment for single-GPU inference
    # DeepGaitV2's BaseModel expects distributed to be initialized (for msg_mgr)
    ...
    # Create model (DeepGaitV2 expects full cfg dict, not just model_cfg)
...
def main():
    parser = argparse.ArgumentParser(
        description='Grad-CAM visualization for DeepGaitV2 (Heavy Frozen)',
...
Example:
    cd /path/to/OpenGait
    python gradcam_deepgaitv2.py --config configs/deepgaitv2/DeepGaitV2_part1_heavy_frozen.yaml ...
        """
    )
    parser.add_argument('--config', type=str, required=True,
                        help='Path to DeepGaitV2 config file (e.g., DeepGaitV2_part1_heavy_frozen.yaml)')
    parser.add_argument('--checkpoint', type=str, default=None,
                        help='Path to DeepGaitV2 checkpoint')
    ...
    parser.add_argument('--target_layer', type=str, default='layer4',
                        choices=['layer0', 'layer1', 'layer2', 'layer3', 'layer4', 'all'],
    ...
    model = load_deepgaitv2_model(args.config, args.checkpoint, args.device)
    ...
    if args.input.endswith('.pkl'):
        ...
        # Resize to match DeepGaitV2 expected dimensions (width must be 44 or 88)
        # Handle different input shapes - normalize to [S, H, W] first
        original_shape = silhouette_seq.shape
        
        # Normalize shape to [S, H, W] for resizing
        if len(silhouette_seq.shape) == 3:  # [S, H, W]
            seq_len, height, width = silhouette_seq.shape
            needs_resize = (width not in [44, 88])
        elif len(silhouette_seq.shape) == 4:  # [N, S, H, W] or [S, C, H, W]
            if silhouette_seq.shape[1] > 100:  # Likely [N, S, H, W]
                n, seq_len, height, width = silhouette_seq.shape
                silhouette_seq = silhouette_seq[0]  # Take first batch: [S, H, W]
                needs_resize = (width not in [44, 88])
            else:  # Likely [S, C, H, W]
                seq_len, c, height, width = silhouette_seq.shape
                silhouette_seq = silhouette_seq[:, 0] if c > 0 else silhouette_seq  # Remove channel: [S, H, W]
                needs_resize = (width not in [44, 88])
        elif len(silhouette_seq.shape) == 5:  # [N, C, S, H, W]
            n, c, seq_len, height, width = silhouette_seq.shape
            silhouette_seq = silhouette_seq[0, 0] if c > 0 else silhouette_seq[0]  # Take first batch/channel: [S, H, W]
            needs_resize = (width not in [44, 88])
        else:
            needs_resize = False
        
        # Resize if needed - create both 64x44 for model and 64x64 for visualization
        if needs_resize:
            print(f"Resizing from {height}x{width} to {height}x44 for model and {height}x64 for visualization")
            seq_len, height, width = silhouette_seq.shape
            resized_seq_model = []
            resized_seq_vis = []
            for i in range(seq_len):
                frame = silhouette_seq[i]
                # cv2.resize expects (width, height) format
                # Resize to 44x64 for model
                frame_model = cv2.resize(frame, (44, height), interpolation=cv2.INTER_LINEAR)
                resized_seq_model.append(frame_model)
                # Resize to 64x64 for visualization (matching SwinGait)
                frame_vis = cv2.resize(frame, (64, height), interpolation=cv2.INTER_LINEAR)
                resized_seq_vis.append(frame_vis)
            silhouette_seq = np.stack(resized_seq_model, axis=0)
            silhouette_seq_vis = np.stack(resized_seq_vis, axis=0)
            print(f"Resized from {original_shape} to model: {silhouette_seq.shape}, vis: {silhouette_seq_vis.shape}")
        else:
            # Already correct size for model (44 or 88), but create 64x64 version for visualization
            seq_len, height, width = silhouette_seq.shape
            if width == 64:
                # Already 64x64, use same for visualization
                silhouette_seq_vis = silhouette_seq.copy()
            else:
                # Need to resize to 64x64 for visualization (from 44 or 88)
                # cv2.resize expects (width, height) format
                resized_seq_vis = []
                for i in range(seq_len):
                    frame = silhouette_seq[i]
                    # Resize to 64x64: (width=64, height=64)
                    frame_vis = cv2.resize(frame, (64, 64), interpolation=cv2.INTER_LINEAR)
                    resized_seq_vis.append(frame_vis)
                silhouette_seq_vis = np.stack(resized_seq_vis, axis=0)
                print(f"Created 64x64 visualization version from {height}x{width} model input")
    else:
        ...
        # Load images as 64x64 for visualization (matching SwinGait)
        # Then resize to 64x44 for model input
        vis_size = (64, 64)  # (width, height) - for visualization
        model_size = (44, 64)  # (width, height) - for model input
        images_vis = []  # For visualization (64x64)
        images_model = []  # For model (64x44)
        for f in image_files:
            img = Image.open(f).convert('L')
            # Load as 64x64 for visualization
            img_vis = img.resize(vis_size, Image.LANCZOS)
            images_vis.append(np.array(img_vis))
            # Resize to 44x64 for model input
            img_model = img.resize(model_size, Image.LANCZOS)
            images_model.append(np.array(img_model))
        
        print(f"Loaded images: {vis_size[1]}x{vis_size[0]} (HxW) for visualization, {model_size[1]}x{model_size[0]} (HxW) for model")
        silhouette_seq_vis = np.stack(images_vis, axis=0)  # [S, 64, 64] for visualization
        silhouette_seq = np.stack(images_model, axis=0)  # [S, 64, 44] for model
    
    # Convert to tensor
    if isinstance(silhouette_seq, np.ndarray):
        silhouette_seq = torch.from_numpy(silhouette_seq).float()
    if isinstance(silhouette_seq_vis, np.ndarray):
        silhouette_seq_vis = torch.from_numpy(silhouette_seq_vis).float()
    
    # Normalize to [0, 1]
    if silhouette_seq.max() > 1:
        silhouette_seq = silhouette_seq / 255.0
    if silhouette_seq_vis.max() > 1:
        silhouette_seq_vis = silhouette_seq_vis / 255.0
    
    # DeepGaitV2 expects input to be [N, S, H, W] (4D), not [N, C, S, H, W]
    ...
    # Same for visualization sequence
    if len(silhouette_seq_vis.shape) == 3:  # [S, H, W]
        silhouette_seq_vis = silhouette_seq_vis.unsqueeze(0)  # [1, S, H, W]
    elif len(silhouette_seq_vis.shape) == 5:  # [N, C, S, H, W]
        silhouette_seq_vis = silhouette_seq_vis.squeeze(1)  # [N, S, H, W]
    
    # Ensure it's [N, S, H, W]
    assert len(silhouette_seq.shape) == 4, f"Expected 4D tensor [N, S, H, W], got {silhouette_seq.shape}"
    assert len(silhouette_seq_vis.shape) == 4, f"Expected 4D tensor [N, S, H, W], got {silhouette_seq_vis.shape}"
    
    print(f"Model input shape: {silhouette_seq.shape}")
    print(f"Visualization input shape: {silhouette_seq_vis.shape}")
    ...
    if args.target_layer == 'all':
        layers_to_process = ['layer0', 'layer1', 'layer2', 'layer3', 'layer4']
    else:
    ...
    if participant_id and frailty_class:
        base_filename = f"gradcam_deepgaitv2_heavy_frozen_{participant_id}_{frailty_class}"
    elif participant_id:
        base_filename = f"gradcam_deepgaitv2_heavy_frozen_{participant_id}"
    else:
        base_filename = f"gradcam_deepgaitv2_heavy_frozen_{input_basename}" if input_basename else "gradcam_deepgaitv2_heavy_frozen_output"
    ...
    for layer_name in layers_to_process:
        ...
        # Resize CAM from model size (64x44) to visualization size (64x64)
        cam_vis = cv2.resize(cam, (64, 64), interpolation=cv2.INTER_LINEAR)
        
        # Verify visualization inputs are 64x64
        if len(silhouette_seq_vis.shape) == 4:  # [N, S, H, W]
            vis_h, vis_w = silhouette_seq_vis.shape[2], silhouette_seq_vis.shape[3]
        elif len(silhouette_seq_vis.shape) == 3:  # [S, H, W]
            vis_h, vis_w = silhouette_seq_vis.shape[1], silhouette_seq_vis.shape[2]
        else:
            vis_h, vis_w = silhouette_seq_vis.shape[-2], silhouette_seq_vis.shape[-1]
        print(f"Visualization frame size: {vis_h}x{vis_w}, CAM size: {cam_vis.shape}")
        assert vis_h == 64 and vis_w == 64, f"Visualization frames must be 64x64, got {vis_h}x{vis_w}"
        
        # Visualize using 64x64 frames and CAM
        print(f"Creating visualization for {layer_name}...")
        visualize_gradcam(silhouette_seq_vis, cam_vis, output_path, 
