#!/usr/bin/env python3
"""
Aggregate metrics for SwinGait Part 4a models (B1, B2, B4) across k-folds.
...
# Model configurations: {model_name: {fold: iteration}}
MODEL_CONFIGS = {
    'B1': {  # Frozen CNN
        1: 1500,
        2: 9000,
...
MODEL_DIR_PATTERNS = {
    'B1': 'REDO_Frailty_ccpg_pt4a_swingait_B1_frozen_cnn_fold',
    'B2': 'REDO_Frailty_ccpg_pt4a_swingait_B2_frozen_cnn_with_weights_fold',
    'B4': 'REDO_Frailty_ccpg_pt4a_swingait_B4_unfrozen_cnn_with_weights_fold'
}
...
def find_log_file(model_dir_pattern, fold_num):
    ...
    # Try to find SwinGait subdirectory
    swingait_dir = os.path.join(exp_dir, 'SwinGait')
    if os.path.exists(swingait_dir):
        # Find the model subdirectory
        model_dirs = [d for d in os.listdir(swingait_dir) if os.path.isdir(os.path.join(swingait_dir, d))]
        if model_dirs:
            exp_full_dir = os.path.join(swingait_dir, model_dirs[0])
        else:
            exp_full_dir = swingait_dir
    else:
...
def print_focused_summary(all_results):
    ...
    for model_name in ['B1', 'B2', 'B4']:
        ...
        print(f"{model_name:<10} {acc_str:<20} {kappa_str:<20} {auc_macro_str:<20} {auc_micro_str:<20}")
...
def print_summary(all_results):
    ...
    print(f"\n{'='*80}")
    print("SWINGAIT PART 4A METRICS SUMMARY (FULL)")
    print(f"{'='*80}\n")
...
def main():
    ...
    json_file = os.path.join(output_dir, 'swingait_part4a_aggregated_metrics.json')
