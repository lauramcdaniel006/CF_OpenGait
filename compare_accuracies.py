#!/usr/bin/env python3
"""
Compare accuracies from first 5000 iterations of two configs:
...
def extract_accuracies(log_file, max_iter=5000):
...
# Paths to log files - use the ones with most iterations
...
print("=" * 80)
print("Comparing accuracies for first 5000 iterations")
print("=" * 80)
...
# Extract accuracies
acc1 = extract_accuracies(log1, max_iter=5000)
acc2 = extract_accuracies(log2, max_iter=5000)
...
if len(common_iters) == 0:
    ...
    print(f"Config 1 iterations: {sorted(acc1.keys())[:10]}...")
    print(f"Config 2 iterations: {sorted(acc2.keys())[:10]}...")
else:
    # Compare accuracies
    matches = 0
    mismatches = 0
    ...
    if mismatches > 0:
        ...
        for iter_num in sorted(common_iters):
            ...
            diff = abs(a1 - a2)
            if diff >= 1e-6:
                print(f"  Iter {iter_num:5d}: Config1={a1:.6f}, Config2={a2:.6f}, Diff={diff:.6f}")
                count += 1
                if count >= 10:
                    break
    else:
    ...
    print("\n" + "=" * 80)
    print("Sample comparisons (every 500 iterations):")
    print("=" * 80)
    print(f"{'Iter':<8} {'Config1 Acc':<15} {'Config2 Acc':<15} {'Match':<8}")
    print("-" * 80)
    for iter_num in sorted(common_iters):
        if iter_num % 500 == 0 or iter_num <= 1000:
            a1 = acc1[iter_num]
            a2 = acc2[iter_num]
            match = "✓" if abs(a1 - a2) < 1e-6 else "✗"
            print(f"{iter_num:<8} {a1:<15.6f} {a2:<15.6f} {match:<8}")
